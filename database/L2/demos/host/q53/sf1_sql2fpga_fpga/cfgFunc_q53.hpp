...
void get_cfg_dat_JOIN_INNER_TD_829930_gqe_join(ap_uint<512>* hbuf) {
    ...
    shuffle2_cfg(7, 0) = 6; // i_manufact_id
    shuffle2_cfg(15, 8) = 0; // ss_sold_date_sk
    shuffle2_cfg(23, 16) = 1; // ss_store_sk
    shuffle2_cfg(31, 24) = 2; // ss_sales_price
    ...
    shuffle3_cfg(7, 0) = 0; // i_manufact_id
    shuffle3_cfg(15, 8) = 1; // ss_sold_date_sk
    shuffle3_cfg(23, 16) = 2; // ss_store_sk
    shuffle3_cfg(31, 24) = 3; // ss_sales_price
    ...
    shuffle4_cfg(7, 0) = 0; // i_manufact_id
    shuffle4_cfg(15, 8) = 1; // ss_sold_date_sk
    shuffle4_cfg(23, 16) = 2; // ss_store_sk
    shuffle4_cfg(31, 24) = 3; // ss_sales_price
...
void get_cfg_dat_JOIN_INNER_TD_747576_gqe_join(ap_uint<512>* hbuf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
...
void get_cfg_dat_JOIN_INNER_TD_6839131_gqe_join(ap_uint<512>* hbuf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
...
void get_cfg_dat_Aggregate_TD_5872697_gqe_aggr(ap_uint<32>* buf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: Aggregate
    // Operation: ListBuffer(i_manufact_id#241, d_qoy#130, MakeDecimal(sum(UnscaledValue(ss_sales_price#1219)),17,2) AS sum_sales#3544, MakeDecimal(sum(UnscaledValue(ss_sales_price#1219)),17,2) AS _w0#3549)
    // Input Table: ListBuffer(i_manufact_id#241, ss_sales_price#1219, d_qoy#130)
    // Output Table: ListBuffer(i_manufact_id#241, sum_sales#3544, _w0#3549)
    // Node Depth: 5
    ...
    // eval0
    ap_uint<32> t;
    signed char id[] = {1,0,2,-1,-1,-1,-1,-1};
    for (int c = 0; c < 4; ++c) {
    ...
    shuffle1_cfg(15, 8) = 1; // i_manufact_id
    shuffle1_cfg(23, 16) = 2; // d_qoy#130
    shuffle1_cfg(31, 24) = 8; // sum_sales
    ...
    ap_uint<289> op_eval_1 = 0; // ss_sales_price#1219
    xf::database::dynamicALUOPCompiler<uint32_t, uint32_t, uint32_t, uint32_t>("strm1", 0, 0, 0, 0, op_eval_1);
    for (int i = 0; i < 9; i++) {
    ...
    shuffle2_cfg(15, 8) = 1; // i_manufact_id
    shuffle2_cfg(23, 16) = 2; // d_qoy
    shuffle2_cfg(31, 24) = 3; // sum_sales
    shuffle2_cfg(39, 32) = 8; // _w0
    shuffle2_cfg(47, 40) = -1;
    shuffle2_cfg(55, 48) = -1;
    shuffle2_cfg(63, 56) = -1;
    ...
    // filter -> groupBy
    ap_int<64> shuffle3_cfg;
    shuffle3_cfg(7, 0) = 1; // i_manufact_id#241
    shuffle3_cfg(15, 8) = 2; // d_qoy#130
    ...
    ap_int<64> shuffle4_cfg;
    shuffle4_cfg(7, 0) = 3; // sum_sales#3544
    shuffle4_cfg(15, 8) = 8; // _w0#3549
